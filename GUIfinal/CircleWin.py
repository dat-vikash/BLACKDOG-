
from PyQt4 import *
from PyQt4.QtCore import *
from PyQt4.QtGui import *



from CircleDialog import Ui_CircleDialog



class CircleWin(QWidget, Ui_CircleDialog):

    def __init__(self, parent = None):   
        QWidget.__init__(self, parent)
        self.setupUi(self)        
        
        self.latitude = 0
        self.longitude = 0
        self.radius = None
        
        self.connect(self.commitButton, QtCore.SIGNAL('clicked()'), self.commit)
        self.connect(self.cancelButton, QtCore.SIGNAL('clicked()'), self.cancel)
        self.connect(self.previewButton, QtCore.SIGNAL('clicked()'), self.preview)
        
    def commit(self):
        try:
            self.latitude = abs(int(self.latDEdit.text()))+(float(self.latMEdit.text())/60)+(float(self.latSEdit.text())/3600)
            self.longitude = abs(int(self.longDEdit.text()))+(float(self.longMEdit.text())/60)+(float(self.longSEdit.text())/3600)
        
            if int(self.latDEdit.text())<0:
                self.latitude *=-1
            if int(self.longDEdit.text())<0:
                self.longitude *=-1
            
            self.emit(SIGNAL('commitCircle'))
        except Exception, x:
            error = QErrorMessage(self)
            error.showMessage("Invalid Field Input")
        
    def cancel(self):
        self.emit(SIGNAL('cancel'))   
    def preview(self):
        try:
            self.radius = int(self.radiusEdit.text())
            self.emit(SIGNAL('previewCircle')) 
        except Exception, x:
            error = QErrorMessage(self)
            error.showMessage("Invalid Field Input")       
    def getLatitude(self):
        return self.latitude
    def getLongitude(self):
        return self.longitude
    def getRadius(self):
        self.radius = int(self.radiusEdit.text())
        return self.radius